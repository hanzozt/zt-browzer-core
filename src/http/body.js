...
import { Stream } from '../http/readable-stream/_stream_readable';
...
function HttpBody(body, init = {
	...
	// 	// return `multipart/form-data;boundary=${body.getBoundary()}`;
		...
		// zt._ctx.logger.debug('extractContentType() FormData DETECTED for: %o', body);
...
HttpBody.prototype.extractContentType = function(body) {
	if (body === null) {
		...
		// detect FormData object
		// zt._ctx.logger.info('extractContentType() FormData DETECTED for: %o', body);
		// for (var key of body.keys()) {
		// 	zt._ctx.logger.info('key is: ', key);
		// 	zt._ctx.logger.info('val is: ', body.get(key));
		// 	// form.append(key, body.get(key));
		//  }
		//  zt._ctx.logger.debug('form is: %o', body);
		//  zt._ctx.logger.debug('getHeaders() says: %o', body.getHeaders());
...
/**
 * Write a HttpBody to a WritableStream (e.g. ZitiHttpRequest) object.
 *
 * @param   HttpBody    instance   Instance of HttpBody
 * @return  Void
 */
