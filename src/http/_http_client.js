...
import { connResetException, codes } from './internal/errors';
const {
...
function ClientRequest(input, options, cb) {
  ...
  if (methodIsString && method) {
    if (!HTTPCOMMON.checkIsHttpToken(method)) {
...
function ondrain() {
  ...
  if (msg && !msg.finished && msg[HTTPINTERNAL.kNeedDrain]) {
    msg[HTTPINTERNAL.kNeedDrain] = false;
    msg.emit('drain');
  }
}
...
function socketCloseListener() {
  ...
  if (parser) {
    parser.finish();
    HTTPCOMMON.freeParser(parser, req, socket);
  }
}
...
function socketOnData(d) {  
  ...
  if (ret instanceof Error) {
    HTTPCOMMON.prepareError(ret, parser, d);
...
function tickOnSocket(req, socket) {
  const parser = HTTPCOMMON.parsers.alloc();
  req.socket = socket;
  parser.initialize(HTTPCOMMON.HTTPParser.RESPONSE); 
