...
class Sender {
  ...
  static frame(data, options) {
    ...
    if (merge) {
      BUFFERUTIL.applyMask(data, mask, target, offset, data.length);
      return [target];
    }

    BUFFERUTIL.applyMask(data, mask, data, 0, data.length);
    return [target, data];
  }
  ...
  ping(data, mask, cb) {
    ...
    if (this._deflating) {
      ...
      this.doPing(buf, mask, BUFFERUTIL.toBuffer.readOnly, cb);
    }
  }
  ...
  pong(data, mask, cb) {
    ...
    if (this._deflating) {
      this.enqueue([this.doPong, buf, mask, BUFFERUTIL.toBuffer.readOnly, cb]);
    } else {
      this.doPong(buf, mask, BUFFERUTIL.toBuffer.readOnly, cb);
    }
  }
  ...
  send(data, options, cb) {
    ...
    if (perMessageDeflate) {
      const opts = {
        ...
        opcode,
        mask: options.mask,
        readOnly: BUFFERUTIL.toBuffer.readOnly
      };
